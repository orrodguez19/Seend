...
function renderUserList(users) {
    ...
    publicChat.className = `user-item ${currentChat === 'public' ?
'active' : ''}`;
...
function updateUserStatus(data) {
    ...
    userItems.forEach(item => {
        ...
        if (data.user_id === currentChat) {
            chatStatus.textContent = data.status === 'online' ?
'En línea' : 
                                   data.status === 'typing' ?
'Escribiendo...' : 
                                   'Desconectado';
    ...
    if (currentReply) {
        ...
        replyContent = `
            <div class="reply-container">
                ...
                <div class="reply-text">${text.substring(0, 50)}${text.length > 50 ?
'...' : ''}</div>
            </div>
        `;
    }
...
`
                        <svg class="status-icon" width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path d="M6 6L8.66667 
8.66667L14 3.33333" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(-4 0)"/>
                        </svg>
                    ` : ''}
                </div>
            </div>
