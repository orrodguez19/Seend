...
def get_conversation_messages(user_id):
    ...
    conn.commit()
    c.execute("SELECT u.username as sender, m.message, m.timestamp, m.status FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.conversation_id = ? ORDER BY m.timestamp ASC", (conversation_id,))
    messages = [{"sender": row[0], "message": row[1], "timestamp": row[2], "status": row[3]} for row in c.fetchall()]
    conn.close()
...
def get_public_messages():
    ...
    conn = sqlite3.connect('chat.db')
    c = conn.cursor()
    c.execute("SELECT u.username as sender, pm.message, pm.timestamp FROM public_messages pm JOIN users u ON pm.sender_id = u.id ORDER BY pm.timestamp ASC")
...
def handle_public_message(data):
    ...
    timestamp = time.time()
    c.execute("INSERT INTO public_messages (id, sender_id, message, timestamp) VALUES (?, ?, ?, ?)", (message_id, session['user_id'], message, timestamp))
    conn.commit()
    conn.close()
    emit('public_message', {"sender": sender, "message": message, "timestamp": timestamp}, broadcast=True)
...
def handle_private_message(data):
    ...
    timestamp = time.time()
    c.execute("INSERT INTO messages (id, conversation_id, sender_id, message, timestamp, status) VALUES (?, ?, ?, ?, ?, ?)", 
              (message_id, conversation_id, session['user_id'], message, timestamp, status))
    conn.commit()
    conn.close()
    sender_sid = request.sid
    receiver_sid = next((sid for sid, user in connected_users.items() if user['id'] == receiver_id), None)
    message_data = {"sender": sender, "message": message, "timestamp": timestamp, "conversation_id": conversation_id, "status": status}
...
def handle_message_status(data):
    ...
    conn = sqlite3.connect('chat.db')
    c = conn.cursor()
    c.execute("UPDATE messages SET status = ? WHERE conversation_id = ? AND sender_id != ? AND status != 'seen'", 
    ...
    if sender_sid:
        emit('message_status_update', {"conversation_id": conversation_id, "status": status}, room=sender_sid)
